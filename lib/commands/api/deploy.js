...
module.exports = async function(
  ...
  const environmentExists = checkIfEnvironmentExists(existingEnvironment)
...
function checkIfEnvironmentExists(apiEnvironment) {
  if (!apiEnvironment || !apiEnvironment.bmServerVersion) {
    return false
  }

  // Need to check if the last deployment was done using the
  // old @blinkmobile/server-cli package. If so the version
  // will be a semver version, not the full package name and version
  if (!apiEnvironment.bmServerVersion.startsWith(pkg.name)) {
    return false
  }

  const { version } = parsePackageName(apiEnvironment.bmServerVersion)
